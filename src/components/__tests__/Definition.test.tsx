...
describe('Definition', () => {
  ...
  it('displays definition header', () => {
    render(<Definition wordle="test" definition="test definition" setDefinition={mockSetDefinition} />);
    expect(screen.getByText('Definition')).toBeInTheDocument();
  });

  it('displays definition content when provided', () => {
    const testDefinition = '1. A test word\n2. Another meaning';
    render(<Definition wordle="test" definition={testDefinition} setDefinition={mockSetDefinition} />);
