...
const GameTiles = ({ hardMode }) => {
    ...
    const checkRow = async () => {
        ...
        if (currentTile === 5) {
            try {
                const response = await axios(`http://localhost:8000/check/?word=${guess}`)
    ...
    const flipTile = () => {
        return new Promise((resolve) => {
            ...
            rowTiles.forEach((x, index) => {
                guess.push({ letter: guessRows[currentRow][index], color: 'grey-overlay' })
            })
    ...
    return (
        <>
            ...
            {loser && (
                <LoseMessage>
                    <div className="solution-was-lose">
                        Solution Was
                    </div>
